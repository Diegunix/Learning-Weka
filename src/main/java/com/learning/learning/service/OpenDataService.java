...
public class OpenDataService {
    ...
    private final String URL = "https://opendata.aemet.es/opendata/api/prediccion/especifica/municipio/horaria/";

    private final String FORMAT_NUM = "%%0%dd";
    
    private final DateTimeFormatter f = DateTimeFormatter.ofPattern("HH");

    public Weather getWeather(Integer provincia, Integer localidad) throws IOException, JsonSyntaxException, ParseException {
    ...
    private Weather parseWeather(ResponsePredictionAemet[] dataWeather) throws ParseException {
    ...
    private int getDataPeriod(List<ObjectDataAemet> data, LocalDateTime fecha) {
        for (ObjectDataAemet dataAemet : data) {
            String start = (String) dataAemet.getPeriodo().substring(0,2);
            String end = (String) dataAemet.getPeriodo().substring(2,4);
